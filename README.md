...
A relational database is a way of managing data so that the various data tables built with rows and columns, which can be connected. SQL is Structured Query Language, which is used to interact with such a database.
- [ ] Why do tables need a `primary key`?
Tables need a primary key as a unique identifier for that table.
- [ ] What is the name given to a table column that references the primary key on another table.
Foreign key
- [ ] What do we need in order to have a _many to many_ relationship between two tables.
A junction table to connect these items, and foreign keys.
